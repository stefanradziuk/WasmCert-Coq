...
Lemma nth_error_map: forall {X Y:Type} (l: seq X) n f {fx: Y},
    ...
    {x & List.nth_error l n = Some x /\ f x = fx}.
Proof.
  move => X Y l n.
  generalize dependent l.
...
Lemma func_context_store: forall s i C j x,
    ...
    {a & List.nth_error i.(inst_funcs) j = Some a}.
Proof.
...
Proof.
  ...
  (* TODO is there no way to name hypotheses coming from gen_ind? *)
  ...
  - (* BI_br_table *)
    ...
    rewrite Evcs.
    (* TODO: explicitly name H5 (if possible) *)
  ...
  - (* Call *)
    right. subst.
    simpl in *. clear H1 H2. (* XXX why are those missing? H3 H4 *)
    (* TODO rename e *)
    eapply func_context_store in e; eauto. destruct e as [a e].
  ...
  - (* Get_local *)
    right. invert_typeof_vcs.
    (* TODO naming: i0, e*)
    simpl in i0. simpl in e.
    apply nth_error_map in e.
    destruct e as [x' [HN HType]].
    rewrite length_is_size in i0.
    rewrite size_map in i0.
  ...
  - (* Set_local *)
    right. invert_typeof_vcs.
    simpl in i0.
    rewrite length_is_size in i0. rewrite size_map in i0. rewrite -length_is_size in i0.
  ...
  - (* Set_global *)
    ...
    + unfold supdate_glob in Hs. unfold option_bind in Hs.
      (* TODO bad naming *)
      simpl in i0. simpl in e.
      eapply glob_context_store in e; eauto.
      unfold sglob in e. unfold operations.sglob in e. unfold option_bind in e.
  ...
  - (* Load *)
    ...
    eapply mem_context_store in n; eauto.
    destruct n as [n [HMenInd HMem]].
  ...
  - (* Composition *)
    ...
    + (* Const *)
      (* TODO naming *)
      apply const_es_exists in i. destruct i as [cs HConst].
      ...
      * destruct s0 as [es' HReduce]. (* TODO naming *)
        right.
    ...
    + (* reduce *)
      (* TODO naming *)
      destruct s0 as [s' [vs' [es' [hs' HReduce]]]].
      right.
      rewrite to_e_list_cat.
...
Proof.
  ...
  - invert_e_typing.
    ...
    (* TODO s0 naming (dependent induction) *)
    destruct s0 as [lh2 [HConst [HLF2 HLength]]].
    repeat eexists. repeat split => //; eauto.
    apply lfilled_Ind_Equivalent in HLF2. apply lfilled_Ind_Equivalent.
    instantiate (1 := (LH_rec vs (length ts2) es' lh2 es'')).
    apply LfilledRec => //.
...
Proof.
  ...
  inversion HType as [??????? HFT]. inversion HFT. subst.
  destruct (k<n) eqn: H3 => //=.
  move/ltP in H3.
  ...
  simpl in HLF.
  assert (E : tc_label C1 = [::]). eapply inst_t_context_label_empty. eauto.
  by rewrite E in HLF.
Qed.
...
Proof.
  ...
  (* TODO the naming here is a bit flaky -- subst comes up with X0 (?) *)
  inversion HType as [??????? H X]; subst.
...
Proof.
  ...
  - (* Local *)
    ...
    (* XXX pattern change - \/ / + associativity
     * return to nested pattrn? [ [] | ] ?
     *)
    edestruct IHHType as [H | H]; eauto.
    {
      move => n lh k HLF.
      by eapply s_typing_lf_br in HLF; eauto.
    }
    { unfold return_reduceT in HEMF. unfold not_lf_return.
      move => n lh HContra.
      (* TODO could this be made nicer through automation / redefintion of
       * notT? shouldn't have to apply this explicitly
       *)
      apply Empty_set_imp_False.
      apply HEMF. by eauto.
    }
    + (* Const *)
      (* XXX H is wrong here - it shouln't have the disjunction??? *)
  ...
  - (* Invoke *)
    ...
    + (* Host *)
      ...
      assert (HApply: {hs' & {res & host_application hs s (Tf (map typeof vcs) ts2) h vcs hs' res}}). apply host_application_exists.
  ...
  - (* AI_label *)
    ...
    edestruct IHHType2 as [HTerm | HReduce]; eauto.
    { rewrite upd_label_overwrite. simpl. eauto. }
    { unfold br_reduceT in HEMF.
      move => n lh k HLF.
    ...
    { unfold not_lf_return.
      ...
      unfold not_lf_return in HNRet.
      eapply HNRet.
      apply lfilled_Ind_Equivalent in HContra.
      apply lfilled_Ind_Equivalent.
    ...
    + (* Terminal *)
      ...
      apply terminal_form_v_e in HTerm.
      unfold terminal_form in HTerm. destruct HTerm.
      * (* Const *)
        right.
        exists s, f, es, hs.
        apply r_simple.
    ...
    + (* reduce *)
      (* TODO explicit naming *)
      destruct HReduce as [s' [f' [es' [hs' HReduce]]]].
      right.
      simpl in HReduce.
  ...
  - (* s_typing *)
    ...
    inversion HFT.
    subst.
    edestruct IHHType as [HTerm | HReduce]; eauto.
    { (* Context *)
      ...
      by fold_upd_context. }
    { by instantiate (1 := [::]). }
    + unfold terminal_form in HTerm. destruct HTerm as [HTerm|].
      * (* Const *)
        left. left. split => //.
        apply et_to_bet in HType; try by apply const_list_is_basic.
        simpl in HTerm.
        apply const_es_exists in HTerm. destruct HTerm as [es' HTerm]. subst.
      ...
      * (* AI_trap *)
        left. by right.
    + (* reduce *)
      simpl in HReduce. right. by eauto.
Qed.
...
(* TODO: move this to type_progress_extraction. Leaving this here module for
 * now to avoid dealing with some implicit argument mess. *)
