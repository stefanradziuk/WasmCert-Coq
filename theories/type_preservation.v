...
Proof.
  move => C v t.
  assert (H : be_typing C [:: BI_const (wasm_deserialise (bits v) t)] (Tf [::] [:: typeof (wasm_deserialise (bits v) t)])); first by apply bet_const.
...
(*  induction HType => //. *)
  - (* ety_a *)
    assert (H : es_is_basic (operations.to_e_list bes)); first by apply to_e_list_basic.
    rewrite Hremes in H. by basic_inversion'.
  - (* ety_composition *)
...
Proof.
  ...
  - move => lh tss LI HLF ts2 ts HType HTSSLength.
    ...
    replace (n.+1+length tss) with (n + length tss') in HLF'.
    eapply IHn => //=.
    + by apply HLF'.
    + by apply H12.
    (* replace *)
...
Proof.
  ...
  - (* ety_a *)
    assert (Hesbasic : es_is_basic (operations.to_e_list bes)); first by apply to_e_list_basic.
    rewrite Heqles in Hesbasic. by basic_inversion'.
  - (* ety_composition *)
  ...
  - (* ety_weakening *)
    move => ts2 ts1 HTf.
    inversion HTf. subst.
    edestruct IHHType as [ts' [H1 [H2 H3]]] => //=. subst.
    exists ts'.
    repeat split => //=.
    by rewrite catA.
  - (* ety_local *)
...
Proof.
  ...
  - invert_be_typing.
    by eapply IHHType2 => //=.
  - edestruct IHHType as [ts' [ts'' [H1 H2]]] => //=. subst.
    exists ts', (ts ++ ts'').
    split => //=.
    by rewrite -catA.
Qed.
...
Lemma Lfilled_return_typing: forall n lh vs LI ts s C lab t2s,
    ...
    e_typing s C vs (Tf [::] ts).
Proof.
  induction n; move => lh vs LI ts s C lab t2s HType HConst HLength HLF HReturn; apply lfilled_Ind_Equivalent in HLF; inversion HLF as [|????????? HLF']; subst => //=.
  - repeat rewrite catA in HType.
    apply e_composition_typing in HType.
...
Proof.
  ...
  (* TODO better naming; using this in the meantime for consistency with the previous version *)
  inversion H2 as [??????? H H0 H1].
  inversion H. subst. clear H4. clear H2. clear H.
 ...
 *)
    ...
    + (* ety_a *)
      assert (Hesbasic : es_is_basic (operations.to_e_list bes)); first by apply to_e_list_basic.
      rewrite Hremes in Hesbasic. by basic_inversion'.
    + (* ety_composition *)
      apply extract_list1 in Hremes. destruct Hremes. subst.
      apply et_to_bet in HType1 => //.
      simpl in HType1. apply empty_typing in HType1. subst.
  ...
  - (* Local_const *)
    ...
    + (* ety_weakening *)
      ...
      by eapply IHHType => //.
    + (* ety_local *)
      (* TODO explicitly name s0 (if possible)? *)
      inversion Hremes; inversion H; inversion s0; subst.
      by eapply t_const_ignores_context; eauto.
...
Proof.
  ...
  - invert_be_typing.
    by apply IHHType2 => //=.
  - edestruct IHHType as [ts' [t1s' [t2s' [H1 [H2 [H3 H4]]]]]] => //=. subst.
    exists (ts ++ ts'), t1s', t2s'.
    repeat rewrite -catA.
    by repeat (split => //=).
Qed.
...
Proof.
  ...
  - invert_be_typing.
    by apply IHHType2 => //=.
  - edestruct IHHType as [ts' [tm [ts'' [H1 [H2 [H3 [H4 H5]]]]]]] => //=. subst.
    exists ts', tm, (ts ++ ts'').
    by repeat rewrite -catA.
Qed.
...
Proof.
  ...
  - inversion Hremtf; subst.
    edestruct IHHType => //=.
    destruct s as [C' [H1 [H2 [H3 H4]]]]. subst.
    exists (ts0 ++ x), C'.
    by repeat split => //; rewrite catA.
  - inversion Hremes. subst.
    (* TODO better and/or explicit naming (if possible) *)
    rename e into H, c into H0.
    rewrite H in HNth.
...
Proof.
  ...
  - apply extract_list1 in Hremes. destruct Hremes. subst.
    ...
    by eapply IHHType2 => //=.
  - inversion Hremtf; subst.
    edestruct IHHType as [ts' Hts'] => //=.
    exists (ts ++ ts'). destruct Hts'. subst.
    by split; repeat rewrite catA.
  - inversion Hremes; subst.
    (* TODO explicit naming (if possible) *)
    rename e into H, c into H0.
    rewrite H in HNth.
    inversion HNth; subst; clear HNth.
    inversion H0; subst.
...
Proof.
  ...
  - invert_be_typing.
    by apply IHHType2.
  - edestruct IHHType as [ts' [t [H1 [H2 [H3 [H4 H5]]]]]] => //=. subst.
    exists ts', (tg_t ts').
    repeat split => //=.
    by rewrite -catA.
Qed.
...
Proof.
  ...
  - invert_be_typing.
    by eapply IHHType2.
  - edestruct IHHType as [ts' [H1 [H2 [H3 H4]]]] => //=. subst.
    exists (ts ++ ts').
    by repeat rewrite -catA.
Qed.
...
Lemma lfilled_es_type_exists: forall k lh es les s C tf,
    lfilled k lh es les ->
    e_typing s C les tf ->
    {lab & {t1s & {t2s & e_typing s (upd_label C lab) es (Tf t1s t2s)}}}.
Proof.
  ...
  - invert_e_typing.
    ...
    by rewrite upd_label_unchanged.
  - invert_e_typing.
    edestruct IHHLF as [? H]; first by apply H4.
...
Proof.
  ...
  - (* update glob *)
    ...
    (* TODO explicitly name this earlier *)
    rename e into H.
    ...
    destruct g => //.
    destruct g0 => //.
    simpl in H0. unfold global_agree in H5. simpl in H5.
  ...
  - (* update memory : store none *)
    ...
    (* TODO explicit naming *)
    rename e into H0, e0 into H1.
    simpl in H1.
  ...
  - (* again update memory : grow_memory *)
    ...
    simpl in H0.
    assert (i < length s_mems0)%coq_nat.
    (* TODO e0 is a bad name -- possibly not worth fixing? *)
  ...
  - (* r_local *)
    apply Local_typing in HType.
    destruct HType as [ts [H1 [H2 H3]]]. subst.
    inversion H2 as [??????? H ? H1]. inversion H. subst.
...
Proof.
  ...
  - convert_et_to_bet.
    ...
    (* TODO e0 bad naming but HReduce has 25 disj branches.
     * Better to leave it like this than name explicitly?
     * This is also the case in a few places below. *)
    rewrite e0.
    rewrite set_nth_map => //.
    by rewrite set_nth_same_unchanged.
  - assert (Het :
      {lab' & {t1s' & {t2s' &
        e_typing s (upd_label (upd_label (upd_local_return C (tc_local C ++ map typeof f.(f_locs)) ret) lab) lab') es (Tf t1s' t2s')}}});
        first eapply lfilled_es_type_exists; eauto.
    destruct Het as [lab' [t1s' [t2s' Het]]].
...
Lemma inst_typing_func: forall s i j C a x,
  ...
  {cl & List.nth_error s.(s_funcs) a = Some cl}.
Proof.
...
Proof.
  ...
  - (* Invoke native *)
    invert_e_typing.
    eapply Invoke_func_native_typing in e; eauto.
    destruct e as [ts2 [C2 [H9 [H10 [H11 H12]]]]]. subst.
  ...
  - (* Invoke host *)
    ...
    {
      destruct host_instance. apply host_application_respect in h0 => //.
      unfold types_agree.
      clear.
      induction vcs => //=.
      by apply/andP => //=.
  ...
  - (* Get_local *)
    ...
    apply bet_weakening_empty_1.
    rewrite e in HNth. inversion HNth; subst.
    by apply bet_const.
  - (* Set_local *)
    convert_et_to_bet.
    replace [::BI_const v; BI_set_local i] with ([::BI_const v] ++ [::BI_set_local i]) in HType => //=.
  ...
  - (* Get_global *)
    ...
    apply ety_a'; auto_basic => //=.
    assert (H0 : tc_local C = [::]); first by eapply inst_t_context_local_empty; eauto.
  ...
  - (* Load None *)
    ...
    apply ety_a'; auto_basic.
    simpl.
    assert (H2 : be_typing C [::BI_const (wasm_deserialise bs t)] (Tf [::] [:: typeof (wasm_deserialise bs t)])); first by apply bet_const.
    rewrite catA.
    apply bet_weakening_empty_1.
  ...
  - (* Store None *)
    + convert_et_to_bet.
      ...
      move/andP in H11. destruct H11. move/eqP in H. subst.
      apply ety_a'; auto_basic => //=.
      apply bet_weakening_empty_both.
      by apply bet_empty.
  - (* Store Some *)
  ...
  - (* r_label *)
    ...
    induction k; move => lab tx ty les' les HType lh HLF1 HLF2;
    apply lfilled_Ind_Equivalent in HLF1; apply lfilled_Ind_Equivalent in HLF2.
    ...
    (* TODO give these Hs more meaningful names
     * this naming is weird but it matches the old autogenerated names *)
    + inversion HLF1 as [|???????? H0 H1 H H2 H3 H4].
      inversion HLF2 as [|???????? H6 H7 H5 H8 H9 H10]. subst.
      inversion H8. subst. clear H8.
      clear H6.
      apply lfilled_Ind_Equivalent in H1. apply lfilled_Ind_Equivalent in H7.
      apply e_composition_typing in HType.
      destruct HType as [ts0 [t1s0 [t2s0 [t3s0 [H2 [H3 [H4 H5]]]]]]]. subst.
      apply e_composition_typing in H5.
