...
Proof.
  ...
  have Dcl: forall vs, decidableT (const_list vs).
  { move=> vs. by apply: is_true_decidableT. }
 ...
 l1 l2 l3. left. by exists (n, l1, l2, l3).
  ...
  - move=> [[vs es''] [E1 [C Ex]]].
    ...
    have D1: forall es' lh lh' n0, decidableT (lfilledInd 0 lh (fes' n0 lh') es').
    { move=> ? ? ? ?. by apply: D0. }
...
Proof.
  ...
  - move=> vsh esh.
    have: pickable2 (fun vs es'' => (es' = vs ++ es ++ es'') /\ (const_list vs) /\ (vs = vsh /\ es'' = esh)).
    {
      ...
      - move=> ? ?.
        repeat apply decidable_and; apply eq_comparable.
    }
    case.
...
Proof.
  ...
  - move=> nE. right. move=> [lh I]. exfalso. apply: nE. inversion I. subst. by repeat eexists.
Defined.
